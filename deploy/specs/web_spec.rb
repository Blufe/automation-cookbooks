...
describe_recipe 'deploy::web' do
  ...
  describe 'files' do
    it 'should create a logrotate with the required contents' do
      node[:deploy].each do |app, deploy|
        if deploy[:application_type].eql?('static')
          file("/etc/logrotate.d/automation_app_#{app}").must_exist.with(:mode, '644').and(
              :owner, 'root').and(:group, 'root')
          file("/etc/logrotate.d/automation_app_#{app}").must_include "#{deploy[:deploy_to]}/shared/log"
        end
      end
    end
  end
